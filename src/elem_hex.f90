module elem_hex
   ...
   contains  
      ...
      subroutine hex27(s,t,z,N,dN) ! HEX27 element
         ...
         N(  13) = 0.25d0*sl*tl*z2
         dN(1,13) = 0.25d0*s1*tl*z2
         dN(2,13) = 0.25d0*sl*t1*z2
         dN(3,13) = 0.25d0*sl*tl*z4
         N(  14) = 0.25d0*sq*tl*z2
         dN(1,14) = 0.25d0*s3*tl*z2
         dN(2,14) = 0.25d0*sq*t1*z2
         dN(3,14) = 0.25d0*sq*tl*z4
         N(  15) = 0.25d0*sq*tp*z2
         dN(1,15) = 0.25d0*s3*tp*z2
         dN(2,15) = 0.25d0*sq*t3*z2
         dN(3,15) = 0.25d0*sq*tp*z4
         N(  16) = 0.25d0*sl*tp*z2
         dN(1,16) = 0.25d0*s1*tp*z2
         dN(2,16) = 0.25d0*sl*t3*z2
         dN(3,16) = 0.25d0*sl*tp*z4
         N(  17) = 0.25d0*s2*tl*zp
         dN(1,17) = 0.25d0*s4*tl*zp
         dN(2,17) = 0.25d0*s2*t1*zp
         dN(3,17) = 0.25d0*s2*tl*z3
         N(  18) = 0.25d0*sq*t2*zp
         dN(1,18) = 0.25d0*s3*t2*zp
         dN(2,18) = 0.25d0*sq*t4*zp
         dN(3,18) = 0.25d0*sq*t2*z3
         N(  19) = 0.25d0*s2*tp*zp
         dN(1,19) = 0.25d0*s4*tp*zp
         dN(2,19) = 0.25d0*s2*t3*zp
         dN(3,19) = 0.25d0*s2*tp*z3
         N(  20) = 0.25d0*sl*t2*zp
         dN(1,20) = 0.25d0*s1*t2*zp
         dN(2,20) = 0.25d0*sl*t4*zp
         dN(3,20) = 0.25d0*sl*t2*z3
         N(  21) = 0.5d0*s2*t2*zl
         dN(1,21) = 0.5d0*s4*t2*zl
         dN(2,21) = 0.5d0*s2*t4*zl
         dN(3,21) = 0.5d0*s2*t2*z1
         N(  22) = 0.5d0*s2*tl*z2
         dN(1,22) = 0.5d0*s4*tl*z2
         dN(2,22) = 0.5d0*s2*t1*z2
         dN(3,22) = 0.5d0*s2*tl*z4
         N(  23) = 0.5d0*sq*t2*z2
         dN(1,23) = 0.5d0*s3*t2*z2
         dN(2,23) = 0.5d0*sq*t4*z2
         dN(3,23) = 0.5d0*sq*t2*z4
         N(  24) = 0.5d0*s2*tp*z2
         dN(1,24) = 0.5d0*s4*tp*z2
         dN(2,24) = 0.5d0*s2*t3*z2
         dN(3,24) = 0.5d0*s2*tp*z4
         N(  25) = 0.5d0*sl*t2*z2
         dN(1,25) = 0.5d0*s1*t2*z2
         dN(2,25) = 0.5d0*sl*t4*z2
         dN(3,25) = 0.5d0*sl*t2*z4
         N(  26) = 0.5d0*s2*t2*zp
