module time_integ
      ...
      contains
              ...
              subroutine rk_2(nelem,npoin,ndime,ndof,nbnodes,nstep,ngaus,nnode, &
                              ldof,lbnodes,connec,Ngp,gpcar,Ml,Mc,gpvol,dt, &
                              rho,u,q,pr,E,Tem,e_int)
                      ...
                      do istep = 1,nstep
                         ...
                         write(*,*) '            (MASS)'
                         call mass_convec(nelem,ngaus,npoin,nnode,ndime,connec,Ngp,gpcar,gpvol,q_1,Rmass_4)
                         call lumped_solver_scal(npoin,Ml,Rmass_4)
                         aux_mass = Rmass_4
                         rho_4(:) = rho(:)-(dt/1.0d0)*aux_mass(:)

                         write(*,*) '            (MOMENTUM)'
                         call mom_convec(nelem,ngaus,npoin,nnode,ndime,connec,Ngp,gpcar,gpvol,u_1,q_1,pr_1,Rmom_4)
                         call lumped_solver_vect(npoin,ndime,Ml,Rmom_4)
                         aux_mom = Rmom_4
                         q_4(:,:) = q(:,:)-(dt/1.0d0)*aux_mom(:,:)
                         q_4(lbnodes,2) = 0.0d0
                         do ipoin = 1,npoin
                            u_4(ipoin,:) = q_4(ipoin,:)/rho_4(ipoin)
                         end do
                         ...
                         write(*,*) '            (ENERGY)'
                         call ener_convec(nelem,ngaus,npoin,nnode,ndime,connec,Ngp,gpcar,gpvol,u_1,pr_1,E_1,Rener_4)
                         call lumped_solver_scal(npoin,Ml,Rener_4)
                         aux_ener = Rener_4
                         E_4(:) = E(:)-(dt/1.0d0)*aux_ener
